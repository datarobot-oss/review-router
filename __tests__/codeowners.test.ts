...
const fixtureContent = fs.readFileSync(path.join(__dirname, "fixtures", "CODEOWNERS"), "utf8");
...
describe("mapFilesToTeams", () => {
  ...
  it("reports unowned files when no CODEOWNERS pattern matches", () => {
    const noDefaultEntries = parseCodeowners("/src/ @datarobot-community/platform-team\n");
  ...
  it("reports unowned files when default owner is also an individual", () => {
    const individualDefaultEntries = parseCodeowners("* @johndoe\ndocs/ @janedoe\n");
  ...
  it("handles files with multiple team owners", () => {
    ...
    expect(result.teamFiles.get("customer-engineering")).toContain("src/shared/utils.ts");
    expect(result.teamFiles.get("platform-team")).toContain("src/shared/utils.ts");
  });
  ...
  it("matches glob patterns in directory names", () => {
    ...
    const result = mapFilesToTeams(["skills/datarobot-app-framework-cicd/README.md"], globEntries);
  ...
  it("extracts team slugs from any org prefix", () => {
    const multiOrgEntries = parseCodeowners("* @datarobot-oss/team-a\n/infra/ @datarobot/team-b\n");
    const result = mapFilesToTeams(["src/app.py", "infra/main.tf"], multiOrgEntries);
