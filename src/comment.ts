...
export function buildOwnershipComment(ownership: OwnershipMap, orgName: string): string {
  ...
  if (ownership.unownedFiles.length > 0) {
    ...
    lines.push("");
    for (const file of ownership.unownedFiles) { lines.push(`- \`${file}\``); }
    lines.push("");
    lines.push("</details>");
  }
  lines.push("");
...
export async function upsertComment(octokit: Octokit, owner: string, repo: string, prNumber: number, body: string): Promise<void> {
